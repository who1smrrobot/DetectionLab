# Purpose: Joins a Windows host to the ecorp.local domain which was created with "create-domain.ps1".
...
$hostname = $(hostname)
$user = "ecorp.local\vagrant"
$pass = ConvertTo-SecureString "vagrant" -AsPlainText -Force
$DomainCred = New-Object System.Management.Automation.PSCredential $user, $pass

# Place the computer in the correct OU based on hostname
If (($hostname -eq "wef") -or ($hostname -eq "exchange")) {
  Add-Computer -DomainName "ecorp.local" -credential $DomainCred -OUPath "ou=Servers,dc=windomain,dc=local" -PassThru
  ...
  Add-Computer -DomainName "ecorp.local" -credential $DomainCred -OUPath "ou=Workstations,dc=windomain,dc=local"
} Else {
  Add-Computer -DomainName "ecorp.local" -credential $DomainCred -PassThru
}
