...
If (($hostname -eq "wef") -or ($hostname -eq "exchange")) {
  Add-Computer -DomainName "ecorp.local" -credential $DomainCred -OUPath "ou=Servers,dc=ecorp,dc=local" -PassThru
  ...
  Add-Computer -DomainName "ecorp.local" -credential $DomainCred -OUPath "ou=Workstations,dc=ecorp,dc=local"
} Else {
