...
If ($env:COMPUTERNAME -eq "DC") {
  ...
  Add-Content 'c:\\windows\\system32\\drivers\\etc\\hosts' '        192.168.38.104    win10.ecorp.local'
}
Else {
  Add-Content 'c:\\windows\\system32\\drivers\\etc\\hosts' '        192.168.38.102    dc.ecorp.local'
  Add-Content 'c:\\windows\\system32\\drivers\\etc\\hosts' '        192.168.38.102    ecorp.local'
}
